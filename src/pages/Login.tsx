...
const Login: React.FC = () => {
  ...
  const handleLogin = async (data: any) => {
    ...
    setError(null);
    try {
      const result = await login(data.email || data.phone, data.password, data.role || 'user');
      if (result.success) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100">
      <Navigation />
      <AuthForm mode="login" onAuth={handleLogin} loading={loading} error={error} setError={setError} />
    </div>
  );
};
